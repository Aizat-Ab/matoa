...
//img
import image1 from '../../assets/way/image1.png';
import image2 from '../../assets/way/image2.png';
import image3 from '../../assets/way/image3.png';
import image4 from '../../assets/way/image4.png';
// import a from '../../assets/hero/watch.png';
import choose1 from '../../assets/way/model1.png';
import choose2 from '../../assets/way/model2.png';
...
const Way = ({ img, title, text, price, oldPrice, id }) => {
    const [nav, setNav] = React.useState(3);
    const navWord = (i) => {
        setNav(i)
    }
    const navWords = ['Home /', 'Product /', 'Watches /', 'Way Kambas Mini Ebony'];
    const a = {
        img, 
        title, 
        text, 
    ...
    const add = () => {
        dispatch(addCartAction(a))
    }

    
    return (
        <div className='way'>
            <ul className='way__nav'>
            {navWords.map((item, index)=>{
                return <li onClick={()=>navWord(index) ? 0 :  <Link to="/"/>} className={classNames({active: nav === index})}>{item}</li>
            })}
            </ul>
            <div className='way__hero'>
                <div className='way__left row'>
                    <div className='way__images'>
                        <img width='70' src={image1} alt="" />
                        <img width='92' src={image2} alt="" />
                        <img width='92' src={image3} alt="" />
                        <img width='107' src={image4} alt="" />
                    </div>
                    <img className='way__img' src={img} alt="" />
                </div>
                <div className='way__right'>
                    <div className='way__title'>{title}</div>
                    <p className='way__old-price'>Rp {oldPrice}</p>
                    <p className='way__new-price'>Rp {price}</p>
                    <p className='way__model'>Choose Model</p>
                    <img src={choose1} alt="" />
                    <img src={choose2} alt="" />
                    <div className='way__btn row'>
                        <button className='way__increment'>-</button>
                        <span className='way__volume'>1</span>
                        <button className='way__decrement'>+</button>
                        <button onClick={add}  className='hero__btn'><svg width="22" height="19" viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
